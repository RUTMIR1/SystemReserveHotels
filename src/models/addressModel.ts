...
export class Address{
    ...
    static async getAddressById(id:string | null=null):Promise<AddressDto>{
        if(!id) throw new MissingParameterException('Address id is required');
        let [rows]:RowDataPacket[] = await querySql(`SELECT id, country, province, city, house_number,
            floor FROM Address WHERE id = ? LIMIT 1`, [id]);
        if(rows.length === 0) throw new NotFoundException('Addres not found');
    ...
    static async updateAddress(id:string | null=null, address:Partial<AddressType>):Promise<AddressDto>{
        if(!id) throw new MissingParameterException('Address id is required');
        if(!address) throw new MissingParameterException('Address data is required');
        const keys:string[] = Object.keys(address);
        if(keys.length === 0) throw new MissingParameterException('No fields found for update address');
        ...
        if(rows.length === 0) throw new NotFoundException('Address not found for update');
