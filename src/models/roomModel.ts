...
import { ValidationExisting } from "../utils/utilModel.js";

export class Room{

    static async validateExisting(room:RoomType | Partial<RoomType>):Promise<ValidationExisting>{
            if(room.categories){
...
INNER JOIN Category c2 WHERE rc2.room_id = r.id AND c2.id = rc2.category_id) AS categories FROM Room r 
    ...
    static async createRoom(room:RoomType | null = null):Promise<string>{
        ...
        const validationExisting:ValidationExisting = await this.validateExisting(room);
    ...
    static async getRoomsByCategory(category:string | null=null):Promise<RoomDto[]>{
        if(!category) throw new MissingParameterException('category is required', [{field:'category', message:'category is required'}]);
        const [rows]:RowDataPacket[] = await querySql(`SELECT DISTINCT r.id, r.name, r.price, r.description, r.image_url, r.state, 
            (SELECT JSON_ARRAYAGG(JSON_OBJECT('id', c2.id, 'name', c2.name)) FROM RoomCategory rc2 
            INNER JOIN Category c2 WHERE rc2.room_id = r.id AND c2.id = rc2.category_id) AS categories FROM Room r 
    ...
    static async getRoomsByState(state:string | null=null):Promise<RoomDto[]>{
        if(!state) throw new MissingParameterException('state is required', [{field:'state', message:'state'}]);
        const [rows]:RowDataPacket[] = await querySql(`SELECT DISTINCT r.id, r.name, r.price, r.description, r.image_url, r.state, 
        (SELECT JSON_ARRAYAGG(JSON_OBJECT('id', c2.id, 'name', c2.name)) FROM RoomCategory rc2 
        INNER JOIN Category c2 WHERE rc2.room_id = r.id AND c2.id = rc2.category_id) AS categories FROM Room r 
             INNER JOIN RoomCategory rc ON r.id = rc.room_id INNER JOIN Category c ON rc.category_id = c.id WHERE r.state = ?`, [state]);
        return rows.map((el:any)=> new RoomDto(el));
    }
}